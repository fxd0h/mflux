...
- **Release notes harvest from PR bodies; CHANGELOG.md is no longer hand-edited**: every PR now carries a fenced `release-note` block (CI enforces it), a new ungated `draft-notes` job collects the blocks merged since the last release into a draft GitHub Release grouped by label, and whoever approves the `pypi` deployment reviews and edits that draft before the gated job publishes it verbatim. The release tooling stops parsing CHANGELOG.md (`ChangelogParser` removed), release-prep PRs shrink to the version bump, and this is the last hand-written entry in this file. (#673)
