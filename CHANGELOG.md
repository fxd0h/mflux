...
- **`--config-from-metadata` replays runs made without `--base-model` again**: the sidecar stored `str(None)`, the literal string `"None"`, for every run with a builtin model, and since 0.19.0 validates `--base-model` against the resolver's own list (#592) the restore fed that string to the validator and every such replay exited with `argument --base-model: invalid choice: 'None'`. The sidecar now stores `null` (or the base a custom checkpoint resolved to), and the restore treats both `null` and the legacy `"None"` as nothing to restore, so sidecars already on disk replay too. (#695)
...
- **Release notes harvest from PR bodies; CHANGELOG.md is no longer hand-edited**: every PR now carries a fenced `release-note` block (CI enforces it), a new ungated `draft-notes` job collects the blocks for the commits since the last release tag into a draft GitHub Release grouped by label, and whoever approves the `pypi` deployment reviews and edits that draft before the gated job publishes it verbatim. The release tooling stops parsing CHANGELOG.md (`ChangelogParser` removed) and release-prep PRs shrink to the version bump. (#685)
